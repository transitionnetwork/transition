...
// JK - ideally a variable, but really need a safe way to refer to main site
DEFINE("MAIN_SITE_URL", 'http://www.transitionnetwork.org/');
...
* @return unknown_type
*/
function transition_links($links, $attributes = array('class' => 'links')) {
...
function transition_breadcrumb($breadcrumb) {
  if (!empty($breadcrumb)) {
 ...
 */
function transition_username($object) {
...
function transition_preprocess_user_picture(&$variables) {
  ...
  if (variable_get('user_pictures', 0)) {
    ...
    if (isset($picture)) {
      ...
      // HACK - JK this is to ensure the correct file comes from the right domain... Assumes stuff it shouldn't - example:
      //$picture =  "http://workspaces.transitionnetwork.org/sites/workspaces.transitionnetwork.org/files/sites/default/files/user-pics/picture-1.jpg";
      if (!module_exists('content_profile')) {
        // drop start of path, cut off to files
        $workspaces_pos = strpos($picture, "sites/workspaces.");
        if ($workspaces_pos) {
          $picture = substr($picture, strpos($picture, "sites/workspaces."));
          $picture = MAIN_SITE_URL . substr($picture, strpos($picture, "files/") + 6);
        }
        else
          $picture = MAIN_SITE_URL . $picture;
      }
      // END HACK
      ...
      if (!empty($account->uid) && user_access('access user profiles')) {
        if (module_exists('content_profile')) {
          ...
          // no CP, probabaly on workspaces, use simplified version
          $attributes = array(
            'attributes' => array('title' => t('View user profile.')),
            'html' => TRUE,
            'absolute' => TRUE);
          $variables['picture'] = l($variables['picture'], MAIN_SITE_URL . "user/$account->uid", $attributes);
        }
      }
    }
  }
 ...
 * Overridden to use content profile links.
 */
function transition_advanced_forum_user_picture($account) {
 ...
 */
function transition_status_messages($display = NULL) {
  $output = '';
...
function transition_status_report($requirements) {
  $i = 0;
 ...
 * 
 */
function transition_form_element($element, $value) {
 ...
 * 
 */
function transition_content_multiple_values($element) {
 ...
 * 
 */
function transition_button($element) {
 ...
 */
function transition_search_theme_form($form) {
  $form['submit']['#type'] = 'image_button';
  $form['submit']['#src'] = drupal_get_path('theme', 'transition') . '/images/search.png';
 ...
 */
function transition_widgets_views_query_alter($view) {
 ...
 */
function transition_widgets_web_widgets_render_widget() {
  drupal_add_css(drupal_get_path('theme', 'transition') . 'widget/widgets.css', 'widget');
}

/**
 * Implementation of hook_theme_registry_alter().
 */
function transition_widgets_theme_registry_alter(&$theme_registry) {
  $path = drupal_get_path("theme", "transition") . '/widget';
